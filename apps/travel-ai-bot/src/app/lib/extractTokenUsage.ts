...
export function extractTokenUsage(responseMetadata: any): TokenUsage | undefined {
  ...
  if (responseMetadata.usage) {
    ...
    // Support both chat and realtime shapes:
    // - chat: prompt_tokens/completion_tokens/total_tokens
    // - realtime: input_tokens/output_tokens/total_tokens
    const promptTokens = (typeof u.prompt_tokens === 'number') ? u.prompt_tokens
                        : (typeof u.input_tokens === 'number') ? u.input_tokens
                        : undefined;
    const completionTokens = (typeof u.completion_tokens === 'number') ? u.completion_tokens
  ...
  // Format 3: Direct properties at metadata root (chat-like)
  ...
  // Format 3b: Direct properties at metadata root (realtime-like)
  if (responseMetadata.input_tokens && responseMetadata.output_tokens) {
    return {
      promptTokens: responseMetadata.input_tokens,
      completionTokens: responseMetadata.output_tokens,
      totalTokens: (responseMetadata.total_tokens || (responseMetadata.input_tokens + responseMetadata.output_tokens)),
      audioDuration
    };
  }
