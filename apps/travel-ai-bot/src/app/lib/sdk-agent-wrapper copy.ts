 ...
 * 
 * For voice mode, we use the OpenAI approach:
 * 1. Create RealtimeAgent objects directly from database configs
 * 2. Use global user interaction state (not per-agent wrappers)
 * 3. Let SDK handle agent handoff naturally via agent_handoff events
 * 4. Use session.update() to switch active agents (no session recreation)
 */

// Global user interaction state - shared across all agents
let globalUserInteracted = false;

// Global function to mark user interaction
...
// Function to set the handoff trigger
...
// Function to trigger agent handoff
...
// Direct agent creation function (no wrapper class)
...
// Legacy wrapper class (kept for compatibility with text mode)
