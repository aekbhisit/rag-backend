...
/**
 * /services/channels/health - Channel Health Monitoring Endpoint
 * ============================================================
 * 
 * PURPOSE:
 * - Monitors health status of all communication channels
 * - Provides system-wide health overview and individual channel status
 * - Used for debugging, monitoring, and fallback decisions
 * 
 * USAGE:
 * - GET: Returns overall system health with all channel statuses
 * - POST: Allows specific channel checks and forced health checks
 * - Called by: APIEndpointTester.tsx, MultiChannelInterface.tsx, ChannelManager.ts
 * 
 * CHANNELS MONITORED:
 * - realtime: Voice chat and WebRTC functionality
 * - normal: Text chat and API completions
 * - human: Human staff handoff and availability
 * 
 * FEATURES:
 * - Real-time health checking with response times
 * - Error tracking and capability reporting
 * - System-wide health aggregation
 * - Individual channel testing
 * - Forced health check capabilities
 * 
 * EXAMPLES:
 * - GET /services/channels/health (overall system status)
 * - POST /services/channels/health (with action: "check_specific", channel: "realtime")
 * - POST /services/channels/health (with action: "force_health_check")
 * 
 * HEALTH STATUSES:
 * - active: Channel is fully operational
 * - degraded: Channel has issues but partially functional
 * - offline: Channel is completely unavailable
 */
...
export async function GET(req: NextRequest) {
  try {
    ...
    // Check all channels in parallel for efficiency
...
async function checkRealtimeChannelHealth(): Promise<ChannelHealthStatus> {
  ...
  try {
    ...
    // This tests the voice chat session creation capability
...
async function checkNormalAPIChannelHealth(): Promise<ChannelHealthStatus> {
  ...
  try {
    ...
    // This tests the text chat API functionality
...
async function checkHumanStaffChannelHealth(): Promise<ChannelHealthStatus> {
  ...
  try {
    ...
    // This tests the human handoff functionality
