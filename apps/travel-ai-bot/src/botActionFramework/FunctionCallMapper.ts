...
const functionToActionMap: Record<string, ActionType> = {
  ...
  // Content extraction
  extractContent: ActionType.EXTRACT_CONTENT,
  ...
  navigateTravel: ActionType.NAVIGATE_PAGE,
};
...
export function getBotActionFunctionDefinitions(): any[] {
  return [
    ...
    },
    {
      type: "function",
      name: "extractContent",
      description: "Extracts structured content from the currently visible UI to help formulate a reply. Use when you need on-screen data (lists, cards, details).",
      parameters: {
        type: "object",
        properties: {
          scope: { type: "string", description: "Logical area to extract, e.g., 'tours', 'places', 'taxi', 'help'." },
          limit: { type: "number", description: "Maximum items to return (default 10)" },
          detail: { type: "boolean", description: "If true, include detailed fields where available" }
        }
      }
    },
