          ...
          <div className="px-6 py-2 border-b border-stone-200 bg-white">
            <nav className="ta-breadcrumb" aria-label="Breadcrumb">
              <ol className="flex items-center gap-2">
                ...
                <li>
                  <span className="inline-flex items-center px-3 py-1.5 rounded-md bg-emerald-600 text-white">{currentLanguage==='th'?'แผนการเดินทาง':'Itinerary'}</span>
                </li>
              </ol>
            </nav>
          </div>
              ...
              {[1,2,3].map((d)=> (
                <div key={d} className="border border-stone-200 rounded-lg p-4 bg-white">
                  <div className="flex items-center justify-between">
                    ...
                    <button className="text-sm px-2 py-1 rounded border border-stone-300 hover:border-emerald-400">{currentLanguage==='th'?'ส่งออก':'Export'}</button>
                  </div>
