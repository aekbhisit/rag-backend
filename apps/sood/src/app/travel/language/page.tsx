          ...
          <div className="px-6 py-2 border-b border-stone-200 bg-white">
            <nav className="ta-breadcrumb" aria-label="Breadcrumb">
              <ol className="flex items-center gap-2">
                ...
                <li>
                  <span className="inline-flex items-center px-3 py-1.5 rounded-md bg-emerald-600 text-white">{currentLanguage==='th'?'ภาษา':'Language'}</span>
                </li>
              </ol>
            </nav>
          </div>
          ...
          <div id="ta-phrase-list" className="flex-1 min-h-0 overflow-y-auto p-6 space-y-3 text-sm">
            {PHRASES.map((p)=> (
              <div key={p.id} className="border border-stone-200 rounded-lg bg-white p-3 flex items-center justify-between">
                ...
                <button id="ta-phrase-play" onClick={()=>play(p)} className="px-3 py-1.5 rounded bg-emerald-600 text-white hover:bg-emerald-700">{currentLanguage==='th'?'เล่นเสียง':'Play'}</button>
              </div>
            ))}
          </div>
        </section>
