...
import type { Event as EventType } from '@/app/types/marketplace';
import { 
...
const MarketplacePage: React.FC = () => {
  ...
  // Set chatbox state to minimized by default
  ...
  useEffect(() => {
    const fetchEvents = async () => {
      setLoading(true);
      try {
        const response = await fetch(`/api/marketplace/events?category=${selectedCategory}&page=${currentPage}&limit=${eventsPerPage}&sortBy=${sortBy}`);
        const data = await response.json();
        
        if (data.success) {
          setEvents(data.data.events);
          setTotalEvents(data.data.totalCount);
  ...
  const eventSections = [
    ...
    },
    {
      id: 'upcoming',
      title: '⏰ Upcoming Soon',
      subtitle: 'Don\'t miss these Thailand events',
      color: 'from-blue-500 to-cyan-500', 
      events: featuredEvents.filter(e => new Date(e.date) > new Date('2024-11-01') && new Date(e.date) <= new Date('2024-12-31'))
    },
    {
      id: 'trending',
      title: '✨ Trending Now',
      subtitle: 'Rising in popularity across Thailand',
      color: 'from-purple-500 to-pink-500',
      events: featuredEvents.filter(e => e.featured && e.soldTickets > 100)
    }
  ];
  ...
  const handleMenuNavigation = (section: MenuSection) => {
    ...
    // When navigating to a menu section, reset category to 'all'
  ...
  const handleCategorySelect = (categoryId: string) => {
    ...
    // When selecting a category, reset menu section to 'home'
      ...
      <div className="w-full px-4 sm:px-6 lg:px-8 py-4 md:py-6">
        <div className="flex flex-col lg:flex-row gap-4 lg:gap-6 max-w-full">
          ...
          <main className="flex-1 min-w-0">
            ...
            {(activeMenuSection === 'home' || selectedCategory !== 'all') && (
              <>
                {/* Enhanced Event Sections - Improved Responsive Design */}
                {/* Only show featured sections when no category is selected and on home page */}
                {activeMenuSection === 'home' && selectedCategory === 'all' && eventSections.map((section) => (
                            ...
                            <div className="p-4 sm:p-5">
                              ...
                              <div className="flex flex-col sm:flex-row gap-2 sm:space-x-3 sm:gap-0">
                                ...
                                <button 
                                  ...
                                  className="sm:flex-shrink-0 px-4 py-2.5 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 transition-colors text-sm font-medium"
                                >
                                  Save Event
                                </button>
                              </div>
            ...
            {/* About Us Page */}
            {activeMenuSection === 'about' && selectedCategory === 'all' && (
            ...
            {/* Contact Us Page */}
            {activeMenuSection === 'contact' && selectedCategory === 'all' && (
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-8">
                <h1 className="text-3xl font-bold text-gray-800 mb-6 flex items-center">
                  📞 Contact Us
                </h1>
            ...
            {activeMenuSection === 'terms' && selectedCategory === 'all' && (
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-8">
                <h1 className="text-3xl font-bold text-gray-800 mb-6 flex items-center">
                  📋 Terms & Conditions
                </h1>
            ...
            {/* Privacy Policy Page */}
            {activeMenuSection === 'privacy' && selectedCategory === 'all' && (
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-8">
                <h1 className="text-3xl font-bold text-gray-800 mb-6 flex items-center">
                  🔒 Privacy Policy
                </h1>
          ...
          {/* Right Sidebar - Chat - Now floating like dashboard */}
          <aside className="fixed bottom-6 right-6 z-50">
            <div className={`transition-all duration-300 w-72 md:w-80`}>
              <MarketplaceChatBot 
