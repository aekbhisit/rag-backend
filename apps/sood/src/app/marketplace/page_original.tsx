...
const MarketplacePage: React.FC = () => {
  ...
  // Set mobile chatbox state to minimized/closed by default 
  const [showMobileChatbox, setShowMobileChatbox] = useState(false);
  // Add state for desktop chatbox minimization
  const [isDesktopChatMinimized, setIsDesktopChatMinimized] = useState(true);
          ...
          <aside className="hidden lg:block w-96 flex-shrink-0">
            <div className="bg-white rounded-xl shadow-lg border border-gray-200 sticky top-24 overflow-hidden" style={{ height: 'calc(100vh - 140px)' }}>
              ...
              <div className="h-[calc(100%-80px)]">
                <MarketplaceChatBot 
                  className="h-full" 
                  isMinimized={isDesktopChatMinimized}
                  onToggleMinimize={() => setIsDesktopChatMinimized(!isDesktopChatMinimized)}
                />
              </div>
            </div>
          </aside>
        ...
        <div className="lg:hidden fixed inset-0 z-40 bg-black/20 p-4 pt-28 pb-6">
          <div className="h-full bg-white rounded-xl shadow-2xl flex flex-col overflow-hidden">
            ...
            <div className="flex-1 overflow-hidden">
              <MarketplaceChatBot 
                className="h-full"
                isMinimized={false}  // When showing mobile chatbox, it's always expanded
                onToggleMinimize={() => setShowMobileChatbox(false)}  // Closing when minimize is clicked
              />
            </div>
          </div>
        </div>
