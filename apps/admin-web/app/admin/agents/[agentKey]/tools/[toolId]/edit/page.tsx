...
export default function EditToolPage() {
  ...
  const toolId = decodeURIComponent(String(params?.toolId || ''));
  ...
  async function loadTool() {
    if (!agentKey || !toolId) return;
    
    try {
      setLoading(true);
      setError('');
      
      const res = await fetch(`${apiBase}/agents/${encodeURIComponent(agentKey)}/tools/${encodeURIComponent(toolId)}`);
      ...
      setToolConfig({
        ...
        arg_templates: tool.arg_templates || {},
        guardrails: tool.guardrails || {},
        overrides: tool.overrides || {}
      });

      // Load AI function parameters if they exist
      if (tool.function_name || tool.function_description || tool.function_parameters) {
        setCustomAIFunction({
      ...
      if (tool.parameter_mapping) {
        setParameterMappings(tool.parameter_mapping);
      }
    } catch (err) {
  ...
  // Function to generate request body mapping based on selected tool and parameter mappings
  const generateRequestBodyMapping = () => {
    ...
    // Base templates for different RAG tools
    let baseTemplate = '';
    
    if (selectedTool === 'skill.rag.search') {
      baseTemplate = `{
  ...
  "prompt_params": null
    ...
    } else if (selectedTool === 'skill.rag.place') {
      baseTemplate = `{
  "text_query": "$text_query_str",
  "lat": $lat_num,
  "long": $long_num,
  ...
  "prompt_params": null
    ...
    // Update template with current parameter mappings
  ...
  // Load tool data when component mounts
  useEffect(() => {
    if (agentKey && toolId) {
      loadTool();
    }
  ...
  async function saveTool() {
    ...
    try {
      const toolData = {
        ...
        function_name: customAIFunction.name,
        function_description: customAIFunction.description,
        function_parameters: customAIFunction.parameters,
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 space-y-4">
      <nav className="text-sm text-gray-600">
        ...
        <span className="mx-2">/</span>
        <span className="text-gray-900">Edit Tool</span>
      </nav>

      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-gray-900">Edit Tool for Agent: {agentKey}</h1>
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <div className="bg-blue-50 border border-blue-200 rounded p-3 mb-4">
                        <h5 className="font-medium text-blue-800 mb-2">🎯 Parameter Constraints & Enums</h5>
                          ...
                          <div key={paramName} className="border border-gray-200 rounded-lg p-4 bg-white shadow-sm">
                            ...
                            <div className="grid grid-cols-12 gap-4 items-center mb-3">
                              ...
                              <div className="col-span-3">
                                <label htmlFor={`param-type-${paramName}`} className="block text-xs font-medium text-gray-700 mb-1">Data Type</label>
                                <div className="relative">
                                  <select
                                    id={`param-type-${paramName}`}
                                        ...
                                        parameters: {
                                          ...
                                          properties: {
                                            ...
                                            [paramName]: {
                                              ...
                                              type: newType,
                                              ...(e.target.value !== 'string' && { enum: undefined }) // Clear enum if type is not string
                                            }
                                          }
                                        }
                                      }));
                              ...
                              <div className="col-span-6">
                                ...
                                <div className="flex items-center gap-2">
                                  <input 
                                    ...
                                    disabled={!(Array.isArray(param.type) ? param.type[0] === 'string' : param.type === 'string')}
                                  />
                                  <button
                                    type="button"
                                    onClick={() => {
                                    ...
                                    title="Get enum suggestions based on parameter name"
                                    disabled={!(Array.isArray(param.type) ? param.type[0] === 'string' : param.type === 'string')}
                                  >
                                    💡
                                  </button>
                                ...
                                <div className="flex-1">
                                  <select 
                                    ...
                                    onChange={(e) => {
                                      ...
                                      // Update request body mapping
                                      if (newMapping && newMapping !== 'static' && newMapping !== 'config') {
                                        const paramType = customAIFunction.parameters.properties[paramName]?.type;
                                        const isString = Array.isArray(paramType) ? paramType[0] === 'string' : paramType === 'string';
                                        const value = isString ? `"$${paramName}"` : `$${paramName}`;
                                        
                                        let updatedBody = toolConfig.arg_defaults.request_body || '';
              ...
              {selectedTool && selectedTool.startsWith('skill.rag.') && (
                <div className="space-y-4">
                  ...
                  <div className="bg-green-50 border border-green-200 rounded p-4">
                    ...
                    <div className="space-y-2">
                      <label className="block text-sm font-medium text-green-800">Request Body Mapping</label>
                      <div className="text-xs text-green-600 mb-1">JSON with $parameter mapping</div>
                      <textarea
                        ...
                        className="w-full h-40 border border-green-300 rounded px-3 py-2 font-mono text-sm resize-y bg-white"
                        placeholder="Enter JSON request body with $parameter mapping..."
                      />
                      <p className="text-xs text-green-600">
                        Use <code>$parameter_name</code> syntax to map AI function parameters. All fields are included - map only what you need, others get default values.
                      </p>
                    </div>
                  </div>
                </div>
